...
public class Task2 {
    ...
    public static int task2D_recursive(int[] arr, int i) {
        ...
        if (i<arr.length) {
          sum += arr[i];
          return sum + task2D_recursive(arr, i+1);}
        else {
          return sum;}
    }
    ...
    public static int task2F_recursive(int[] arr, int i) {
        return oddpr(arr, i) - evensm(arr, i);
    }
    
    public static int oddpr(int[] arr, int i) {
      if (i<arr.length) {
        if (arr[i]%2 != 0) {
          return arr[i] * oddpr(arr, i+1);}
        else {
          return 1 * oddpr(arr, i+1);}}
      else {
        return 1;}
    }
    
    public static int evensm(int[] arr, int i) {
      if (i<arr.length) {
        if (arr[i]%2 == 0) {
          return arr[i] + evensm(arr, i+1);}
        else {
          return 0 + evensm(arr, i+1);}}
      else {
        return 0;}
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("The Array: "+Arrays.toString(arr));
        System.out.println("Expected Output: "+35);
        System.out.print("Your Output    : "+task2F_recursive(arr, 0));
